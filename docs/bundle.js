(function () {
  ...
  function G() {
    return HexDigits[Math.floor(Math.random() * 15)];
  } // Good Enough (TM)
  ...
  function useProvidedId(idFromProp, allowBackup) {
    var idFromContext = F(RandomIdContext);
    var backupId = useRandomId();
    return idFromProp || idFromContext || (allowBackup == "backup" ? backupId : undefined);
  }
  var RandomIdContext = D("");
  function ProvideId(_ref) {
    var children = _ref.children,
        id = _ref.id;
    var backupId = useRandomId();
    return v$1(RandomIdContext.Provider, {
      value: id || backupId
    }, children);
  }

  var PendingModeContext = D("disabled");
  function SetPendingMode(props) {
    return v$1(PendingModeContext.Provider, {
      value: props.mode,
      children: props.children
    });
  }
  function usePendingMode() {
    return F(PendingModeContext);
  }
  ...
  function useStableCallback(fn) {
    ...
    _(ref, function () {
      return fn;
    }); // Assign fn to ref.current (currentFunc) in async-safe way

    return s(function () {
      var currentFunc = ref.current;

      if (!currentFunc) {
        return undefined; //throw new Error('Callback retrieved from useStableCallback() cannot be called from useLayoutEffect().')
      }

      return currentFunc.apply(void 0, [].slice.call(arguments));
    }).current;
  }

  var AsyncConvertError = Symbol("AsyncConvertError");
  function useAsyncEventHandler(_ref) {
    var convertEvent = _ref.convertEvent,
        asyncHandler = _ref.asyncHandler;
    var forceUpdate = useForceUpdate(); // This is used to keep track of whatever handler was called most recently if there's one that's already running.
    ...
    var pendingPromiseFuncRef = s(null); // This is used in order to always show the most recently entered value to the user 
    // while there's a handler running (and probably overwriting any parent value props right before a new one runs)

    var latestCapturedEventRef = s(undefined); // Standard async management stuff

    var _useState = l(false),
        fulfilled = _useState[0],
        setFulfilled = _useState[1];

    var _useState2 = l(undefined),
        error = _useState2[0],
        setError = _useState2[1];

    var _useState3 = l(false),
        pending = _useState3[0],
        setPending = _useState3[1]; // Used for children to be able to display spinners on a delay and such


    var _useState4 = l(0),
        startedTime = _useState4[0],
        setStartedTime = _useState4[1];

    var syncHandler = useStableCallback(function (e) {
      ...
      var selfPromiseFunc = function selfPromiseFunc(info, staleEvent) {
        setStartedTime(+new Date());
        forceUpdate();

        try {
          var probablyAPromise = asyncHandler == null ? void 0 : asyncHandler(info, staleEvent);

          if (probablyAPromise && "then" in probablyAPromise) {
            setPending(true);
            return probablyAPromise.then(function (_) {
              setError(undefined);
              setFulfilled(true);
            })["catch"](function (ex) {
      ...
      var selfPromiseFuncObj;

      try {
        selfPromiseFuncObj = convertEvent(e);
      } catch (ex) {
        selfPromiseFuncObj = AsyncConvertError;
      }

      if (!(selfPromiseFuncObj instanceof Symbol) || selfPromiseFuncObj != AsyncConvertError) {
        latestCapturedEventRef.current = selfPromiseFuncObj;
        if (pending) pendingPromiseFuncRef.current = [selfPromiseFunc, selfPromiseFuncObj, e];else selfPromiseFunc(selfPromiseFuncObj, e);
      }
    });
    var ret = {
      ...
      latestConvertedValue: latestCapturedEventRef.current //ContextProvider: null as any as typeof ContextProvider
  ...
  var Contexts = {
    ...
    Fulfilled: D(false),
    Error: D(undefined),
    StartedTime: D(undefined),
    LatestValue: D(undefined)
  };
  function useIsPending() {
    return F(Contexts.Pending);
  }
  function useLatestValue() {
    return F(Contexts.LatestValue);
  }
  function ProvideAsyncHandlerInfo(_ref2) {
    var children = _ref2.children,
        pending = _ref2.pending,
        fulfilled = _ref2.fulfilled,
        error = _ref2.error,
        startedTime = _ref2.startedTime,
        latestConvertedValue = _ref2.latestConvertedValue;
    return v$1(Contexts.Pending.Provider, {
      value: pending
    }, v$1(Contexts.Fulfilled.Provider, {
      value: fulfilled
    }, v$1(Contexts.Error.Provider, {
      value: error
    }, v$1(Contexts.StartedTime.Provider, {
      value: startedTime
    }, v$1(Contexts.LatestValue.Provider, {
      value: latestConvertedValue
    }, children)))));
  }

  var _excluded$a = ["onFocus", "onBlur"];
  /**
  ...
  function useHasFocus(p) {
    var userOnFocus = p.onFocus,
        userOnBlur = p.onBlur,
        props = _objectWithoutPropertiesLoose(p, _excluded$a);

    var _useState = l(false),
        hasFocus = _useState[0],
        setHasFocus = _useState[1];

    var onFocus = A$1(function (e) {
      userOnFocus == null ? void 0 : userOnFocus.bind(e.target, e);
      setHasFocus(true);
    }, [userOnFocus]);
    var onBlur = A$1(function (e) {
      userOnBlur == null ? void 0 : userOnBlur.bind(e.target, e);
      setHasFocus(false);
    }, [userOnBlur]);
    return _extends({
      onFocus: onFocus,
  ...
  var _excluded$9 = ["id", "value", "checked", "convert", "type", "onInput", "disabled", "readOnly", "childrenPost", "childrenPre", "hasFocus"];
  /**
  ...
  function useLostFocusWhilePending(_ref) {
    var pending = _ref.pending,
        hasFocus = _ref.hasFocus;

    var _useState = l(false),
        lostFocusWhilePending = _useState[0],
        setLostFocusWhilePending = _useState[1];
  ...
  var Input = forwardElementRef(function Input(p, ref) {
    var _extends2;

    var _useHasFocus = useHasFocus(p),
        id = _useHasFocus.id,
        value = _useHasFocus.value,
        checked = _useHasFocus.checked,
        convert = _useHasFocus.convert,
        type = _useHasFocus.type,
        userOnInput = _useHasFocus.onInput,
        disabled = _useHasFocus.disabled,
        readOnly = _useHasFocus.readOnly,
        childrenPost = _useHasFocus.childrenPost,
        childrenPre = _useHasFocus.childrenPre,
        hasFocus = _useHasFocus.hasFocus,
        props = _objectWithoutPropertiesLoose(_useHasFocus, _excluded$9); // For checkboxes, we pretend we never have focus so that they're always immediately disabled when changed.
    // (In contrast to, say, text inputs, where the user can type freely until focus is lost)


    if (["radio", "checkbox"].includes(type)) hasFocus = false; // These controls don't support the readonly attribute

    var pendingMode = usePendingMode();
    if (["radio", "checkbox", "range"].includes(type) && pendingMode == "readOnly") pendingMode = "disabled";
    var prop = ["radio", "checkbox"].includes(type) ? "checked" : "value";
    var valueOrChecked = prop == "value" ? value : checked;

    var _useAsyncEventHandler = useAsyncEventHandler({
      asyncHandler: userOnInput,
      convertEvent: convert != null ? convert : defaultConvert
    }),
        pending = _useAsyncEventHandler.pending,
        onInput = _useAsyncEventHandler.syncHandler,
        fulfilled = _useAsyncEventHandler.fulfilled,
        startedTime = _useAsyncEventHandler.startedTime,
        error = _useAsyncEventHandler.error,
        latestConvertedValue = _useAsyncEventHandler.latestConvertedValue;

    var lostFocusWhilePending = useLostFocusWhilePending({
      pending: pending,
      hasFocus: hasFocus
    });

    if (lostFocusWhilePending) {
      if (pendingMode == "disabled") disabled = true;else if (pendingMode == "readOnly") readOnly = true;
    }

    id = useProvidedId(id, "backup");
    return v$1(ProvideAsyncHandlerInfo, {
      pending: pending,
      error: error,
      latestConvertedValue: latestConvertedValue,
      fulfilled: fulfilled,
      startedTime: startedTime
    }, v$1(ProvideId, {
      id: id
    }, childrenPre, v$1("input", _extends({}, props, {
      id: id,
      ref: ref,
      type: type,
      readOnly: readOnly,
      disabled: disabled
    }, (_extends2 = {}, _extends2[prop] = pending ? latestConvertedValue != null ? latestConvertedValue : valueOrChecked : valueOrChecked, _extends2), {
      onInput: onInput
    })), childrenPost));
  });

  function defaultConvert(_ref2) {
  ...
  function convertEvent$3(e) {
    var target = e.target;
    return target.checked;
  }

  var InputCheckbox = forwardElementRef(function InputCheckbox(p, givenRef) {
    var _useRefElement = useRefElement(),
        element = _useRefElement.element,
        useRefElementProps = _useRefElement.useRefElementProps;

    var _useRefElementProps = useRefElementProps(_extends({}, p, {
      ref: givenRef
    })),
        checkedOrIndeterminate = _useRefElementProps.checked,
        props = _objectWithoutPropertiesLoose(_useRefElementProps, _excluded$7);

    var checked = checkedOrIndeterminate === true;
    var indeterminate = checkedOrIndeterminate === "indeterminate";
    y(function () {
      if (element) element.indeterminate = indeterminate;
    }, [element, indeterminate]);
    return v$1(Input, _extends({
      value: undefined,
      type: "checkbox",
      convert: convertEvent$3
    }, props, {
      checked: checked
    }));
  });

  var _excluded$6 = ["value", "onInput"],
      _excluded2$3 = ["valueR", "valueG", "valueB", "onInput"];

  function convertColor(e) {
    return e.target.value;
  }

  var InputColor = forwardElementRef(function InputColor(p, ref) {
    var onInput = p.onInput;
    var modifiedOnInput = A$1(function (color, staleEvent) {
      var r = parseInt(color.substr(1 + 0 * 2, 2), 16);
      var g = parseInt(color.substr(1 + 1 * 2, 2), 16);
      var b = parseInt(color.substr(1 + 2 * 2, 2), 16);
    ...
    if ("value" in p) {
      var value = p.value,
          props = _objectWithoutPropertiesLoose(p, _excluded$6);
      ...
      return v$1(Input, _extends({
        ...
        onInput: modifiedOnInput,
        value: value
      }));
    } else {
      var _props = _objectWithoutPropertiesLoose(p, _excluded2$3);

      var _value = "#" + p.valueR.toString(16).padStart(2, "0") + p.valueG.toString(16).padStart(2, "0") + p.valueB.toString(16).padStart(2, "0");
  ...
  function convertDate(_ref) {
    ...
    return index_umd.Temporal.PlainDate.from(value);
  }

  var InputDate = forwardElementRef(function InputDate(p, ref) {
    var _dateToString;
    ...
    return v$1(Input, _extends({
      ...
      value: (_dateToString = dateToString$1(p.value)) != null ? _dateToString : "",
      max: dateToString$1(p.max),
      min: dateToString$1(p.min)
    }));
  });
  ...
  function convertDateTime(_ref) {
    var target = _ref.target;
    var value = target.value;
    if (value == "" || value == null) return null;
    return index_umd.Temporal.PlainDateTime.from(value);
  }

  var InputDateTime = forwardElementRef(function InputDateTime(p, ref) {
    var _datetimeToString;
    ...
    return v$1(Input, _extends({
      ...
      value: (_datetimeToString = datetimeToString(p.value)) != null ? _datetimeToString : "",
      max: datetimeToString(p.max),
      min: datetimeToString(p.min)
    }));
  });

  function convertEmail(_ref) {
    var _target$value;

    var target = _ref.target;
    return (_target$value = target.value) != null ? _target$value : "";
  }

  var InputEmail = forwardElementRef(function InputEmail(p, ref) {
    var _p$value;

    return v$1(Input, _extends({
      convert: convertEmail,
      type: "email"
    }, p, {
      value: (_p$value = p.value) != null ? _p$value : "",
      ref: ref
    }));
  });

  var _excluded$5 = ["value", "max", "min"];
  ...
  function convertMonth(e) {
    ...
    try {
      var valueAsDate = index_umd.Temporal.PlainYearMonth.from(value);
      return valueAsDate;
    } catch (ex) {
      return undefined;
    }
  ...
  forwardElementRef(function InputMonth(p, ref) {
    var _dateToString, _dateToString2, _dateToString3;

    var props = _objectWithoutPropertiesLoose(p, _excluded$5);
    ...
    return v$1(Input, _extends({
      ...
      value: (_dateToString = dateToString(p.value)) != null ? _dateToString : undefined,
      max: (_dateToString2 = dateToString(p.max)) != null ? _dateToString2 : undefined,
      min: (_dateToString3 = dateToString(p.min)) != null ? _dateToString3 : undefined
    }));
  });

  function convertNumber(_ref) {
    var target = _ref.target;
    var value = target.value;
    if (value == "" || value == null) return null;
    var valueAsNumber = Number(value.trim());
    if (isFinite(valueAsNumber)) return valueAsNumber;
    return undefined;
  }

  var InputNumber = forwardElementRef(function InputNumber(p, ref) {
    var _p$value$toString, _p$value, _p$max, _p$min;
    ...
    return v$1(Input, _extends({
      ...
      value: (_p$value$toString = (_p$value = p.value) == null ? void 0 : _p$value.toString()) != null ? _p$value$toString : "",
      max: (_p$max = p.max) == null ? void 0 : _p$max.toString(),
      min: (_p$min = p.min) == null ? void 0 : _p$min.toString()
    }));
  });

  var _excluded$4 = ["disabled", "value", "name", "children", "onInput"],
      _excluded2$2 = ["id", "childrenPost", "childrenPre", "value", "disabled"];

  function convertEvent$2(e) {
    var target = e.target;
    return target.value;
  }

  function InputRadioGroup(p) {
    var _ref;
    ...
    var disabled = p.disabled,
        ...
        name = p.name,
        children = p.children,
        userOnInput = p.onInput;
        _objectWithoutPropertiesLoose(p, _excluded$4); //const { enteredValueIsValid, handler: parsedUserOnInput, valueAsEntered } = useParsedEvent(convert, value, false, userOnInput);


    var _useAsyncEventHandler = useAsyncEventHandler({
      asyncHandler: userOnInput,
      convertEvent: convertEvent$2
    }),
        pending = _useAsyncEventHandler.pending,
        onInput = _useAsyncEventHandler.syncHandler,
        latestConvertedValue = _useAsyncEventHandler.latestConvertedValue,
    ...
    return v$1(SelectedValueContext$1.Provider, {
      value: (_ref = pending ? latestConvertedValue : value) != null ? _ref : ""
    }, v$1(RadioGroupDisabledContext.Provider, {
      value: pending || disabled || false
    }, v$1(RadioGroupNameContext.Provider, {
      value: name
    }, v$1(OnInputContext.Provider, {
      value: onInput
    }, v$1(ProvideAsyncHandlerInfo, {
      pending: pending,
      error: error,
      latestConvertedValue: latestConvertedValue,
      fulfilled: fulfilled,
  ...
  var SelectedValueContext$1 = D("");
  var RadioGroupDisabledContext = D(false);
  var RadioGroupNameContext = D("");
  var OnInputContext = D(undefined);
  var InputRadio = forwardElementRef(function InputRadio(p, ref) {
    var id = p.id,
        childrenPost = p.childrenPost,
        childrenPre = p.childrenPre,
        value = p.value,
        disabled = p.disabled,
        props = _objectWithoutPropertiesLoose(p, _excluded2$2);

    var name = F(RadioGroupNameContext);
    var selectedValue = useRadioSelectedValue();
    var onInput = F(OnInputContext);
    id = useProvidedId(id, "backup");
    return v$1(ProvideId, {
      id: id
    }, childrenPre, v$1("input", _extends({
      id: id,
      ref: ref,
      type: "radio",
      ...
      value: value,
      onInput: onInput,
      disabled: F(RadioGroupDisabledContext) || disabled
    }, props)), childrenPost);
  });
  function useRadioSelectedValue() {
    return F(SelectedValueContext$1);
  }

  function convertRange(_ref) {
    var target = _ref.target;
    var value = target.value;
    if (value == "" || value == null) return null;
    var valueAsRange = Number(value.trim());
    if (isFinite(valueAsRange)) return valueAsRange;
    return undefined;
  }

  var InputRange = forwardElementRef(function InputRange(p, ref) {
    var _p$value$toString, _p$value;
  ...
  function convertTime(value, e) {
    if (value == "" || value == null) return null;

    try {
      var valueAsDate = index_umd.Temporal.PlainDate.from(value);
      return valueAsDate;
    } catch (ex) {
      return undefined;
    }
  ...
  forwardElementRef(function InputTime(p) {
    return v$1(Input, _extends({
      convert: convertTime,
      type: "time"
    }, p, {
      value: timeToString(p.value),
      max: timeToString(p.max),
      min: timeToString(p.min)
    }));
  });

  var _excluded$3 = ["id", "type", "onClick", "disabled", "children", "childrenPost", "childrenPre"];

  function returnNull() {
    return null;
  }

  var Button = forwardElementRef(function (p, ref) {
    var id = p.id,
        type = p.type,
        userOnClick = p.onClick,
        disabled = p.disabled,
        children = p.children,
        childrenPost = p.childrenPost,
        childrenPre = p.childrenPre,
        props = _objectWithoutPropertiesLoose(p, _excluded$3); // Buttons can't be readonly


    var pendingMode = usePendingMode();
    if (pendingMode == "readOnly") pendingMode = "disabled";

    var _useAsyncEventHandler = useAsyncEventHandler({
      asyncHandler: userOnClick,
      convertEvent: returnNull
    }),
        pending = _useAsyncEventHandler.pending,
        onClick = _useAsyncEventHandler.syncHandler,
        fulfilled = _useAsyncEventHandler.fulfilled,
        startedTime = _useAsyncEventHandler.startedTime,
        error = _useAsyncEventHandler.error,
        latestConvertedValue = _useAsyncEventHandler.latestConvertedValue;
    ...
    id = useProvidedId(id, "backup");
    if (pending && pendingMode == "disabled") disabled = true;
    return v$1(ProvideAsyncHandlerInfo, {
      pending: pending,
      error: error,
      latestConvertedValue: latestConvertedValue,
      ...
      id: id
    }, childrenPre, v$1("button", _extends({}, props, {
      id: id,
      ref: ref,
  ...
  var _excluded$2 = ["id", "childrenPost", "childrenPre", "disabled", "size", "children", "hasFocus", "onChange"],
      _excluded2$1 = ["value", "disabled", "selected"];
  var SelectMulti = forwardElementRef(function SelectMulti(p, ref) {
    var _useHasFocus = useHasFocus(p),
        id = _useHasFocus.id,
        childrenPost = _useHasFocus.childrenPost,
        disabled = _useHasFocus.disabled,
        size = _useHasFocus.size,
        children = _useHasFocus.children,
        userOnChange = _useHasFocus.onChange,
        props = _objectWithoutPropertiesLoose(_useHasFocus, _excluded$2);

    var convertEventForSelect = A$1(function (e) {
      var target = e.target;
      return new Set(Array.from(target.selectedOptions).map(function (option) {
        return option.value;
      }));
    }, []);
    ...
    var _useAsyncEventHandler = useAsyncEventHandler({
      ...
      asyncHandler: userOnChange
        ...
        onChange = _useAsyncEventHandler.syncHandler,
        pending = _useAsyncEventHandler.pending,
        fulfilled = _useAsyncEventHandler.fulfilled,
        startedTime = _useAsyncEventHandler.startedTime,
        error = _useAsyncEventHandler.error,
    ...
    return v$1(ProvideId, {
      ...
      fulfilled: fulfilled,
      startedTime: startedTime
    }, v$1("div", null, pending.toString()), v$1("div", {
      className: "test"
    }, Array.from(latestConvertedValue != null ? latestConvertedValue : []).join(";")), v$1("select", _extends({}, props, {
      ref: ref,
      size: size,
      multiple: true,
      onInput: onChange,
  ...
  var OptionMulti = forwardElementRef(function OptionMulti(p, ref) {
    var value = p.value,
        disabled = p.disabled,
        selected = p.selected,
        props = _objectWithoutPropertiesLoose(p, _excluded2$1); //const [selectionOverride, setSelectionOverride] = useState<null | boolean>(null);
    //const {pending, latestConvertedValue  } = useAsyncEventHandlerInfo<Set<string>>()!;


    var pending = useIsPending();
    var latestConvertedValue = useLatestValue();
    var pendingSelected = pending ? latestConvertedValue == null ? void 0 : latestConvertedValue.has(value) : undefined;
    props.children = value + ":" + (pending ? "PENDING" : "") + " " + (pendingSelected != null ? pendingSelected : "null").toString() + " ?? " + selected.toString();
    return v$1("option", _extends({
      ref: ref,
      selected: pendingSelected != null ? pendingSelected : selected,
      value: value,
      disabled: disabled
    }, props));
  });

  var _excluded$1 = ["id", "disabled", "value", "children", "size", "onInput", "hasFocus"],
      _excluded2 = ["value", "disabled"];

  function convertEvent$1(e) {
    var target = e.target;
    return target.value;
  }

  var SelectSingle = forwardElementRef(function SelectSingle(p, ref) {
    var _useHasFocus = useHasFocus(p),
        id = _useHasFocus.id,
        disabled = _useHasFocus.disabled,
        value = _useHasFocus.value,
        ...
        userOnInput = _useHasFocus.onInput,
        hasFocus = _useHasFocus.hasFocus,
        props = _objectWithoutPropertiesLoose(_useHasFocus, _excluded$1); //    const { enteredValueIsValid, handler: parsedUserOnInput, valueAsEntered } = useParsedEvent(convert, value, hasFocus, userOnInput);


    var _useAsyncEventHandler = useAsyncEventHandler({
      convertEvent: convertEvent$1,
      asyncHandler: userOnInput
    }),
        pending = _useAsyncEventHandler.pending,
        onInput = _useAsyncEventHandler.syncHandler,
    ...
    if (lostFocusWhilePending) {
      if (pendingMode == "disabled" || pendingMode == "readOnly") disabled = true;
    }

    id = useProvidedId(id, "backup");
    return v$1(SelectedValueContext.Provider, {
      value: pending ? latestConvertedValue != null ? latestConvertedValue : value : value != null ? value : ""
    }, v$1(ProvideId, {
      id: id
    }, v$1(ProvideAsyncHandlerInfo, {
      pending: pending,
      ...
      fulfilled: fulfilled,
      startedTime: startedTime
    }, hasFocus.toString(), v$1("select", _extends({}, props, {
      id: id,
      ref: ref,
      size: size,
      multiple: false,
      onChange: onInput,
      disabled: disabled || pending && !hasFocus
    }), children))));
  });
  var SelectedValueContext = D(""); //const OnInputContext = createContext<((e: Event) => void) | undefined>(undefined);

  var OptionSingle = forwardElementRef(function OptionSingle(p, ref) {
    var value = p.value,
        disabled = p.disabled,
        props = _objectWithoutPropertiesLoose(p, _excluded2);

    var selectedValue = F(SelectedValueContext); //const onInput = useContext(OnInputContext)
  ...
  var _excluded = ["id", "value", "onInput", "disabled", "readOnly", "childrenPost", "childrenPre", "hasFocus"];
  var TextArea = forwardElementRef(function TextArea(p, ref) {
    var _ref;
    ...
    var _useHasFocus = useHasFocus(p),
        ...
        childrenPre = _useHasFocus.childrenPre,
        hasFocus = _useHasFocus.hasFocus,
        props = _objectWithoutPropertiesLoose(_useHasFocus, _excluded); // const { enteredValueIsValid, handler: parsedUserOnInput, valueAsEntered } = useParsedEvent(t => `${t.target.value}`, value ?? "", hasFocus, userOnInput);
    ...
    if (pendingMode == "disabled") disabled || (disabled = pending && !hasFocus);else if (pendingMode == "readOnly") readOnly || (readOnly = pending && !hasFocus);
    id = useProvidedId(id, "backup");
    return v$1(ProvideId, {
      id: id
    }, v$1(ProvideAsyncHandlerInfo, {
      ...
      fulfilled: fulfilled,
      startedTime: startedTime
    }, childrenPre, v$1("textarea", _extends({}, props, {
      id: id,
      ref: ref,
      disabled: disabled,
      readOnly: readOnly,
  ...
  function useAsyncState(initialState) {
      const [value, setValue] = l(initialState);
      const delayedSetState = A$1((newValue) => {
      ...
      return [value, delayedSetState];
  }
  const SelectArray = [0, 1, 2, 3, 4, 5, 6];
  const App = () => {
      const [text, setText] = useAsyncState("");
      const [radio, setRadio] = useAsyncState("");
      const [checked, setChecked] = useAsyncState(false);
      const [value, setValue] = useAsyncState(0);
      const [email, setEmail] = useAsyncState("");
      const [date, setDate] = useAsyncState(null);
      const [datetime, setDatetime] = useAsyncState(null);
      const [r, setR] = useAsyncState(0);
      const [g, setG] = useAsyncState(0);
      const [b, setB] = useAsyncState(0);
      ...
      const setColor = A$1(async (value, r, g, b) => {
          return Promise.all([
              setR(r),
              setG(g),
              setB(b),
      ...
      return v$1("div", null,
          v$1(SetPendingMode, { mode: "disabled" },
              v$1("div", null,
                  v$1(Button, { onClick: async () => new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 2000)) }, "Test button")),
              v$1("div", null,
                  checked.toString(),
                  v$1(InputCheckbox, { checked: checked, onInput: (setChecked) })),
              v$1("div", null,
                  value,
                  v$1(InputNumber, { value: value, onInput: (setValue) })),
              v$1("div", null,
                  value,
                  v$1(InputRange, { value: value, onInput: (setValue), min: 0, max: 20 })),
              v$1("div", null,
                  date?.toString(),
                  v$1(InputDate, { value: date, onInput: (setDate) })),
              v$1("div", null,
                  datetime?.toString(),
                  v$1(InputDateTime, { value: datetime, onInput: (setDatetime) })),
              v$1("div", null,
                  email?.toString(),
                  v$1(InputEmail, { value: email, onInput: (setEmail), multiple: true })),
              v$1("div", null,
                  `#${r.toString(16).padStart(2, "0")}${g.toString(16).padStart(2, "0")}${b.toString(16).padStart(2, "0")}`,
                  v$1(InputColor, { valueR: r, valueG: g, valueB: b, onInput: setColor })),
              v$1("div", null,
                  radio,
                  v$1(InputRadioGroup, { name: "radio-test", value: radio, onInput: (setRadio) },
                      v$1(ProvideId, null, v$1(() => v$1(InputRadio, { value: "A", childrenPre: v$1(() => v$1("label", { htmlFor: useProvidedId(null, "no-backup") }, "A"), {}) }), {})),
                      v$1(ProvideId, null, v$1(() => v$1(InputRadio, { value: "B", childrenPre: v$1(() => v$1("label", { htmlFor: useProvidedId(null, "no-backup") }, "B"), {}) }), {})),
                      v$1(ProvideId, null, v$1(() => v$1(InputRadio, { value: "C", childrenPre: v$1(() => v$1("label", { htmlFor: useProvidedId(null, "no-backup") }, "C"), {}) }), {})))),
              v$1("div", null,
                  select,
                  v$1(SelectSingle, { value: select, onInput: (setSelect), size: 3 },
                      v$1(OptionSingle, { value: "0" }, "Item 0"),
                      v$1(OptionSingle, { value: "1" }, "Item 1"),
                      v$1(OptionSingle, { value: "2" }, "Item 2"))),
              v$1("div", null,
                  select,
                  v$1(SelectSingle, { value: select, onInput: (setSelect) },
                      v$1(OptionSingle, { value: "0" }, "Item 0"),
                      v$1(OptionSingle, { value: "1" }, "Item 1"),
                      v$1(OptionSingle, { value: "2" }, "Item 2"))),
              v$1("div", null,
                  multi,
                  v$1(SelectMulti, { size: SelectArray.length, onChange: A$1((selectedValues, e) => { return setMulti(Array.from(selectedValues).join(";")); }, []) }, SelectArray.map(i => v$1(OptionMulti, { value: i.toString(), selected: multiParsed.has(i.toString()) },
                      "Item ",
                      i)))),
              v$1("div", null,
                  v$1(Input, { type: "text", onInput: (setText), value: text })),
              v$1("div", null,
                  v$1(TextArea, { onInput: (setText), value: text, cols: 60, rows: 40 }))));
  };
  S$1(v$1(App, null), document.getElementById("main"));
